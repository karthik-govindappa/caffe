...
parser.add_argument('--pruned_model', type=str, required=True,
    help='Pruned caffemodel of SSD_300x300_VGGNet_VOC0712')
...
output_result_dir = "{}/workspace/corpora/voc/VOCdevkit/results/VOC2007/{}_score/Main".format(os.environ['HOME'], job_name)
...
model_basename = os.path.basename(pretrain_model).replace('.caffemodel', '')
prune_percent = model_basename.split('_Pruned_')[1]
max_iter = model_basename.split('_Pruned_')[0].split("_iter_")[1]
...
with open(job_file, 'w') as f:
  ...
  if solver_param['solver_mode'] == P.Solver.GPU:
    f.write('--gpu {} 2>&1 | tee {}/{}_test_{}_pruned_{}.log\n'.format(gpus, job_dir, model_name, max_iter, prune_percent))
  else:
